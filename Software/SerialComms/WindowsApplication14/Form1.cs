...
namespace WindowsApplication14
{
    public partial class Form1 : Form
    {
        ...
        private void port_DataReceived_1(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            {
                this.BeginInvoke(new SetTextCallback(InboundMessages), new object[] { InputData });
            }
        }
        private void InboundMessages(string text)
        {
            //this.richTextBoxTrace.Text += text;
            // Set textBox
            richTextBoxTrace.SuspendLayout();
            richTextBoxTrace.SelectionColor = Color.Blue;
            richTextBoxTrace.AppendText(Environment.NewLine + DateTime.Now + " TX: " + text);
            richTextBoxTrace.ScrollToCaret();
            richTextBoxTrace.ResumeLayout();


        }

        private void OutboundMessages(string text)
        {
            //this.richTextBoxTrace.Text += text;
            // Set textBox
            richTextBoxTrace.SuspendLayout();
            richTextBoxTrace.SelectionColor = Color.OrangeRed;
            richTextBoxTrace.AppendText(Environment.NewLine + DateTime.Now + " TX: " + text);
            richTextBoxTrace.ScrollToCaret();
            richTextBoxTrace.ResumeLayout();
        ...
        private bool SendMessage(string sMessage)
        {
            cmdSend.BackColor = Color.Transparent;

            if (ComPort.IsOpen)
            {
                ComPort.Write(sMessage + "\n");
                OutboundMessages(sMessage);
                cmdSend.BackColor = Color.LimeGreen;
                return true;
            }
            else
        ...
        {
            cmdSend.BackColor = Color.Transparent;
            string sMessage = "ShowAllEvents";
            SendMessage(sMessage);
        }

        private void cmdCheckEventNow_Click(object sender, EventArgs e)
        {
            cmdSend.BackColor = Color.Transparent;
            string sMessage = "CheckEnvent";
            SendMessage(sMessage);
        }

        private void cmdShowFullMemory_Click(object sender, EventArgs e)
        {
            cmdSend.BackColor = Color.Transparent;
            string sMessage = "ShowFullMemory";
            SendMessage(sMessage);
        }
        ...
        private void richTextBoxTrace_TextChanged(object sender, EventArgs e)
        {
